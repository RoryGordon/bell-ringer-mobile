...
const byte ACC_DATA_X_LSB_ADDR=0x08;
const byte GYR_DATA_Z_LSB_ADDR=0x18;
const byte GRV_DATA_X_LSB_ADDR=0x2E;

//const byte ACC_INDEX=4;
//const byte GYR_INDEX=8; // 6;
//const byte IMU_INDEX=10; // 8;
//const byte SUM_INDEX=14; // 12;

const byte TIME_BYTES = 4;
const byte ACC_BYTES = 4;
const byte GYR_BYTES = 2;
const byte IMU_BYTES = 4;
const byte SUM_BYTES = 1;

const byte ACC_END = TIME_BYTES + ACC_BYTES; // 8
const byte GYR_END = ACC_END + GYR_BYTES;    // 10
const byte IMU_END = GYR_END + IMU_BYTES;    // 14

//const byte LIA_DATA_X_LSB_ADDR=0x28; // linear xyz accn data in 6 bytes starting here (in NDOF fusion mode)
byte buf[IMU_END+1]; //initialise empty array
...
//  Wire.write(ACC_DATA_Y_LSB_ADDR);
  Wire.write(ACC_DATA_X_LSB_ADDR);
  Wire.endTransmission();
  Wire.requestFrom(BNO055_I2C_ADDR, ACC_BYTES); // request 4 bytes for acc data

  for (byte i = TIME_BYTES; i < ACC_END; i++)
  {
  ...
  Wire.endTransmission();
  Wire.requestFrom(BNO055_I2C_ADDR, GYR_BYTES); // request 2 bytes for gyr data

  for (byte i = ACC_END; i < GYR_END; i++)
  {
      buf[i] = (byte) Wire.read(); // read each byte in turn from the i2c bus
      checksum+=buf[i];
  }
  ...
  Wire.endTransmission();
  Wire.requestFrom(BNO055_I2C_ADDR, IMU_BYTES); // request 4 bytes for imu data

  for (byte i = GYR_END; i < IMU_END; i++)
  {
      buf[i] = (byte) Wire.read(); // read each byte in turn from the i2c bus
      checksum+=buf[i];
  }
  ...
  checksum+=buf[23];
  */
  buf[IMU_END] = checksum & 0xFF; //write lowest byte of sum to last element of buf (checksum)

  buf[IMU_END+1] = IMU_END;
  #ifdef SERIAL
  myPacketSerial.send(buf,IMU_END+1); // Send the array. This is terminated with 0x00
  #endif
  #ifdef BLE
  ble.write(buf, IMU_END+1);
  #endif
  /*
  for (byte i = 0; i < 25; i++)
  {
